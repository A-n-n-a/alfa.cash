    ...
    */
    ...
    public struct Grid {
        ...
        public var color: UIColor {
            return ThemeManager.currentTheme == .day ? UIColor(red: 238/255.0, green: 238/255.0, blue: 238/255.0, alpha: 1) : UIColor.white.withAlphaComponent(0.1)
        }
    }
    ...
    override open func draw(_ rect: CGRect) {
        ...
        self.drawingWidth = self.bounds.width - (1.5 * x.axis.inset)
...
//        if let event = event {
//            handleTouchEvents(NSSet(object: touches), event: event)
//        }
//    }
//    
//    
...
//    }
    ...
    fileprivate func getMinimumValue() -> CGFloat {
        var min: CGFloat = dataStore.first?.min() ?? 0
        for data in dataStore {
    ...
    fileprivate func drawYGrid() {
        ...
        let x2: CGFloat = self.bounds.width - x.axis.inset / 2
        var y1: CGFloat
    ...
    fileprivate func drawXLabels() {
        ...
        for (index, _) in xAxisData.enumerated() {
            ...
            if themed {
                label.textColor = ThemeManager.currentTheme.associatedObject.textColor
            } else {
    ...
    fileprivate func drawYLabels() {
        ...
        for i in stride(from: start, through: stop, by: step){
            ...
            let text = step < 1 ? "\(i)" : String(Int(round(i)))
            label.text = text
            self.addSubview(label)
        }
    }
     ...
     * Add line chart
     */
    open func addLine(_ data: [CGFloat], themed: Bool = false) {
        self.themed = themed
