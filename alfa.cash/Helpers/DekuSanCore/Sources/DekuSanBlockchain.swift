...
open class DekuSanBlockchain: Hashable {
    ...
    open var coinPurpose: DekuSanPurpose
    ...
    public init(purpose: DekuSanPurpose = .bip44) {
        self.coinPurpose = purpose
    }

    /// Returns the address associated with a public key.
    open func address(for publicKey: DekuSanPublicKey) -> DecuSanAddress {
        fatalError("Use a specific Blockchain subclass")
    }

    /// Returns the address given its string representation.
    open func address(string: String) -> DecuSanAddress? {
        fatalError("Use a specific Blockchain subclass")
    }

    /// Returns the address given its raw representation.
    open func address(data: Data) -> DecuSanAddress? {
        fatalError("Use a specific Blockchain subclass")
    }

    // MARK: Hashable

    public static func == (lhs: DekuSanBlockchain, rhs: DekuSanBlockchain) -> Bool {
...
public extension DekuSanBlockchain {
    func derive(from extendedPubkey: String, at path: DerivationPath) -> DecuSanAddress? {
        ...
        return self.address(for: DekuSanPublicKey(data: data)!)
    }
}
