...
open class Ethereum: DekuSanBlockchain {
    /// Chain identifier.
    ...
    open override func address(for publicKey: DekuSanPublicKey) -> DecuSanAddress {
        return publicKey.ethereumAddress
    }

    open override func address(string: String) -> DecuSanAddress? {
        return EthereumAddress(string: string)
    }

    open override func address(data: Data) -> DecuSanAddress? {
