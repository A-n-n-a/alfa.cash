...
extension EIP712TypedData {
    ...
    public var typeHash: Data {
        ...
        return DekuSanCrypto.hash(data)
    }

    /// Sign-able hash for an `EIP712TypedData`
    public var signHash: Data {
        let data = Data(bytes: [0x19, 0x01]) +
            DekuSanCrypto.hash(encodeData(data: domain, type: "EIP712Domain")) +
            DekuSanCrypto.hash(encodeData(data: message, type: primaryType))
        return DekuSanCrypto.hash(data)
    }
    ...
    public func encodeData(data: JSON, type: String) -> Data {
        ...
        var values: [ABIValue] = []
        do {
            let typeHash = DekuSanCrypto.hash(encodeType(primaryType: type))
            ...
            if let valueTypes = types[type] {
                try valueTypes.forEach { field in
                    if let _ = types[field.type],
                        ...
                        values.append(try ABIValue(DekuSanCrypto.hash(nestEncoded), type: .bytes(32)))
    ...
    private func makeABIValue(data: JSON?, type: String) -> ABIValue? {
        if (type == "string" || type == "bytes"),
            ...
            return try? ABIValue(DekuSanCrypto.hash(valueData), type: .bytes(32))
        } else if type == "bool",
