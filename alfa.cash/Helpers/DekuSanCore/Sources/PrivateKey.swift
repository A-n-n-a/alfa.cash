...
public final class PrivateKey: Hashable, CustomStringConvertible {
    ...
    public func publicKey(compressed: Bool = false) -> DekuSanPublicKey {
        let pkData: Data
        if compressed {
            pkData = DekuSanCrypto.getCompressedPublicKey(from: data)
        } else {
            pkData = DekuSanCrypto.getPublicKey(from: data)
        }

        return DekuSanPublicKey(data: pkData)!
    }

    /// Signs a hash.
    public func sign(hash: Data) -> Data {
        return DekuSanCrypto.sign(hash: hash, privateKey: data)
    }

    /// Signs a hash, encodes the result using DER.
    public func signAsDER(hash: Data) -> Data {
        return DekuSanCrypto.signAsDER(hash: hash, privateKey: data)
    }
