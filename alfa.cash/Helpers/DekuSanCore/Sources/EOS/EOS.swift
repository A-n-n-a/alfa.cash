...
public final class EOS: DekuSanBlockchain {
    ...
    public override func address(for publicKey: DekuSanPublicKey) -> DecuSanAddress {
        let compressed = publicKey.compressed
        let check = DekuSanCrypto.ripemd160(compressed.data)[0..<4]
        return EOSAddress(data: compressed.data + check)!
    }

    public override func address(string: String) -> DecuSanAddress? {
        return EOSAddress(string: string)
    }

    public override func address(data: Data) -> DecuSanAddress? {
