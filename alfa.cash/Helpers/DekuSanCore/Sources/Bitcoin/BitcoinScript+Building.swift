...
public extension BitcoinScript {
    ...
    public static func buildPayToPublicKeyHash(address: DecuSanBitcoinAddress) -> BitcoinScript {
    ...
    public static func buildPayToPublicKeyHash(_ hash: Data) -> BitcoinScript {
        ...
        data.append(contentsOf: [DekuSanOpCode.OP_DUP, DekuSanOpCode.OP_HASH160])
        data.append(UInt8(hash.count))
        data.append(Data(hash))
        data.append(contentsOf: [DekuSanOpCode.OP_EQUALVERIFY, DekuSanOpCode.OP_CHECKSIG])
    ...
    public static func buildPayToScriptHash(script: BitcoinScript) -> BitcoinScript {
        return buildPayToScriptHash(DekuSanCrypto.sha256ripemd160(script.data))
    }
    ...
    public static func buildPayToScriptHash(_ scriptHash: Data) -> BitcoinScript {
        ...
        var data = Data()
        data.append(DekuSanOpCode.OP_HASH160)
        data.append(UInt8(scriptHash.count))
        data.append(Data(scriptHash))
        data.append(DekuSanOpCode.OP_EQUAL)
        return BitcoinScript(data: data)
    }
    ...
    public static func buildPayToWitnessPubkeyHash(_ hash: Data) -> BitcoinScript {
        ...
        var data = Data()
        data.append(DekuSanOpCode.OP_0)
        data.append(0x14)
        data.append(Data(hash))
        return BitcoinScript(data: data)
    }
    ...
    static func encodeNumber(_ n: Int) -> UInt8 {
        assert(n >= 0 && n <= 16)
        if n == 0 {
            return DekuSanOpCode.OP_0
        }
        return DekuSanOpCode.OP_1 + UInt8(n - 1)
    }
}
