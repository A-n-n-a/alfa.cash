...
open class Bitcoin: DekuSanBlockchain {

    convenience public init(purpose: DekuSanPurpose = .bip84, network: SLIP.Network) {
    ...
    override open func address(for publicKey: DekuSanPublicKey) -> DecuSanAddress {
        switch coinPurpose {
        case .bip44:
    ...
    override public init(purpose: DekuSanPurpose = .bip84) {
        super.init(purpose: purpose)
    }

    open func compatibleAddress(for publicKey: DekuSanPublicKey) -> DecuSanAddress {
        return publicKey.compressed.compatibleBitcoinAddress(prefix: p2shPrefix)
    }

    open func compatibleAddress(string: String) -> DecuSanAddress? {
        return DecuSanBitcoinAddress(string: string)
    }

    open func compatibleAddress(data: Data) -> DecuSanAddress? {
        return DecuSanBitcoinAddress(data: data)
    }

    open func legacyAddress(for publicKey: DekuSanPublicKey) -> DecuSanAddress {
        return publicKey.compressed.legacyBitcoinAddress(prefix: p2pkhPrefix)
    }

    open func legacyAddress(string: String) -> DecuSanAddress? {
        return DecuSanBitcoinAddress(string: string)
    }

    open func legacyAddress(data: Data) -> DecuSanAddress? {
        return DecuSanBitcoinAddress(data: data)
    }
