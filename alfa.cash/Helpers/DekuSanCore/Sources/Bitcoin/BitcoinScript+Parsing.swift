...
public extension BitcoinScript {
    ...
    func getScriptOp(index: inout Int) -> (opcode: UInt8, operand: Data?)? {
        ...
        if opcode > DekuSanOpCode.OP_PUSHDATA4 {
            return (opcode: opcode, operand: nil)
        }

        // Immediate operand
        var size = 0
        if opcode < DekuSanOpCode.OP_PUSHDATA1 {
            size = Int(opcode)
        } else if opcode == DekuSanOpCode.OP_PUSHDATA1 {
            if bytes.endIndex - index < 1 {
                return nil
            }
            size = index
            index += 1
        } else if opcode == DekuSanOpCode.OP_PUSHDATA2 {
            if bytes.endIndex - index < 2 {
                return nil
            }
            size = Int(readLE16(at: index))
            index += 2
        } else if opcode == DekuSanOpCode.OP_PUSHDATA4 {
