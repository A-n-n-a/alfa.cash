...
public extension DekuSanPublicKey {
    /// Returns the public key address with the given prefix.
    public func legacyBitcoinAddress(prefix: UInt8) -> DecuSanBitcoinAddress {
        let hash = Data([prefix]) + bitcoinKeyHash
        return DecuSanBitcoinAddress(data: hash)!
    }

    public func compatibleBitcoinAddress(prefix: UInt8) -> DecuSanBitcoinAddress {
        let witnessVersion = Data(bytes: [0x00, 0x14])
        let redeemScript = DekuSanCrypto.sha256ripemd160(witnessVersion + bitcoinKeyHash)
        let address = DekuSanCrypto.base58Encode([prefix] + redeemScript)
        return DecuSanBitcoinAddress(string: address)!
    }
    ...
    public func cashAddress(redeemScript: BitcoinScript) -> BitcoinCashAddress {
        let payload = Data([BitcoinCashAddress.p2shVersion]) + DekuSanCrypto.sha256ripemd160(redeemScript.data)
        let data = convertBits(payload, from: 8, to: 5)!
    ...
    public var bitcoinKeyHash: Data {
        return DekuSanCrypto.sha256ripemd160(data)
    }
}
