...
public class DecuSanHDWallet {
    ...
    public init(mnemonic: String, passphrase: String = "") {
        seed = DekuSanCrypto.deriveSeed(mnemonic: mnemonic, passphrase: passphrase)
    ...
    public func getExtendedPrivateKey(for purpose: DekuSanPurpose, coin: SLIP.CoinType, version: SLIP.HDVersion) -> String {
    ...
    public func getExtendedPubKey(for purpose: DekuSanPurpose, coin: SLIP.CoinType, version: SLIP.HDVersion) -> String {
        var node = getNode(for: purpose, coin: coin)
        let buffer = [Int8](repeating: 0, count: 128)
        let fingerprint = hdnode_fingerprint(&node)
        hdnode_private_ckd(&node, DerivationPath.Index(0, hardened: true).derivationIndex)
    ...
    private func getNode(for purpose: DekuSanPurpose, coin: SLIP.CoinType) -> HDNode {
